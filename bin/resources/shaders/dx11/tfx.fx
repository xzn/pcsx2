...
void ps_blend(inout float4 Color, inout float4 As_rgba, float2 pos_xy)
{
	...
	if (SW_BLEND)
	{
		...
		float Ad = PS_RTA_CORRECTION ? trunc(RT.a * 128.0f + 0.05f) / 128.0f : trunc(RT.a * 255.0f + 0.1f) / 128.0f;
...
PS_OUTPUT ps_main(PS_INPUT input)
{
	...
	if (SW_AD_TO_HW)
	{
		float4 RT = PS_RTA_CORRECTION ? trunc(RtTexture.Load(int3(input.p.xy, 0)) * 128.0f + 0.05f) : trunc(RtTexture.Load(int3(input.p.xy, 0)) * 255.0f + 0.1f);
