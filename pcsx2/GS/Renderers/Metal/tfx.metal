...
struct PSMain
{
	...
	void ps_blend(thread float4& Color, thread float4& As_rgba)
	{
		...
		if (SW_BLEND)
		{
			...
			float Ad = PS_RTA_CORRECTION ? trunc(current_color.a * 128.25f) / 128.f : trunc(current_color.a * 255.5f) / 128.f;
	...
	MainPSOut ps_main()
	{
		...
		if (SW_AD_TO_HW)
		{
			alpha_blend = SW_AD_TO_HW ? float4(trunc(current_color.a * 128.25f) / 128.f) : float4(trunc(current_color.a * 255.5f) / 128.f);
		}
		else
		{
